...
const {fix, factorial, fibonacci, foldr} = require('./Fix this - 5 kuy');
...
const memoFib = function(cache) { return fix( compose (memoise (cache)) (fibonacci) ); } ( [] ) ;
const fixFoldr = fix(foldr);

describe("Fix this",()=>{
describe("example tests",()=>{
  it("fix",()=>{
    const fixRev = fix( reverse => xs => xs.length ? reverse(xs.slice(1)).concat([xs[0]]) : xs );
    ...
    assert.strictEqual( fixRev("A man, a plan, a canal - Panama!"), "!amanaP - lanac a ,nalp a ,nam A" );
    assert.strictEqual( fixRev("irreversible"), "elbisreverri" );
    assert.strictEqual( fixRev("desserts"), "stressed" );
    assert.  deepEqual( fixRev([3,1,4,1,5,9,2,6,5,3,5,9]), [9,5,3,5,6,2,9,5,1,4,1,3] );
  });
  it("factorial",()=>{
  ...
  it("foldr",()=>{
    const xs = function* xs(xs) { yield* xs; } ( [1,2,3,4] ) ;
    const mul = x => z => x * z() ;
