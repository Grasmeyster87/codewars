    ...
    for (let i = 0; i < arr.length; i++) {
        for (let j = 0; j < arr[i].length; j++) {
            if (arr[i][j] === 'X') {
                ...
                if (i === 0 || arr[i-1][j] !== 'X') perimeter++;
                // Перевіряємо нижню клітинку
                if (i === arr.length - 1 || arr[i+1][j] !== 'X') perimeter++;
                // Перевіряємо ліву клітинку
                if (j === 0 || arr[i][j-1] !== 'X') perimeter++;
                // Перевіряємо праву клітинку
                if (j === arr[i].length - 1 || arr[i][j+1] !== 'X') perimeter++;
            }
        }
    }
