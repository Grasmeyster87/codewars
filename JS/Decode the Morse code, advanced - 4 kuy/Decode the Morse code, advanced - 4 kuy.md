<div class="markdown prose max-w-none mb-8" id="description"><div style="border:1px solid;position:relative;padding:1ex 1ex 1ex 11.1em;"><div style="position:absolute; left:0;top:0;bottom:0; width:10em; padding:1ex;text-align:center;border:1px solid;margin:0 1ex 0 0;color:#000;background-color:#eee;font-variant:small-caps">Part of Series 2/3</div>
<div>
...
<p>Эта ката является частью серии о коде Морзе. Убедитесь, что вы решили <a href="/kata/decode-the-morse-code" data-turbolinks="false" target="_blank">предыдущую часть</a>, прежде чем пробовать эту. После решения этой задачи вы можете перейти к следующей.

В этой задаче вам нужно написать декодер азбуки Морзе для электрического телеграфа.

Электрический телеграф работает по двухпроводной линии с ключом, который при нажатии соединяет провода, что может быть обнаружено удаленной станцией. Азбука Морзе кодирует каждый передаваемый символ как последовательность «точек» (короткие нажатия на клавишу) и «тире» (длительные нажатия на клавишу).</p>
<p>При передаче азбуки Морзе международный стандарт устанавливает следующее:</p>
<ul><li>«Точка» – длительностью 1 единицу времени.</li>
<li>«Тире» – длительностью 3 единицы времени.</li>
<li>Пауза между точками и тире в символе – длительностью 1 единицу времени.</li>
<li>Пауза между символами внутри слова – длительностью 3 единицы времени.</li>
<li>Пауза между словами – длительностью 7 единиц времени.</li></ul>

<p>Однако стандарт не указывает, какой именно «единица времени» является. И на самом деле разные операторы будут передавать с разной скоростью. Любителю может потребоваться несколько секунд для передачи одного символа, опытный профессионал может передавать 60 слов в минуту, а роботизированные передатчики могут работать намного быстрее.</p>
<p>В этой ката мы предполагаем, что прием сообщения осуществляется автоматически оборудованием, которое периодически проверяет линию, и если линия подключена (клавиша на удаленной станции нажата), записывается <code>1</code>, а если линия не подключена (клавиша на удаленной станции нажата), записывается <code>0</code>. После полного приема сообщение попадает к вам для декодирования в виде строки, содержащей только символы <code>0</code> и <code>1</code>.</p>
<p>Например, сообщение <code>HEY JUDE</code>, то есть <code>···· · −·−− &nbsp; ·−−− ··− −·· ·</code> может быть получено следующим образом:</p>
<p><code>1100110011001100000011000000111111001100111111001111110000000000000011001111110011111100111111000000110011001111110000001111110011001100000011</code></p>
<p>Как вы можете видеть, эта передача абсолютно точна в соответствии со стандартом, и оборудование считывало строку ровно два раза на каждую «точку».</p>
<p>Тем не менее, ваша задача состоит в том, чтобы Реализуйте две функции:</p>
<ol>
<li>Функция <code>decodeBits(bits)</code>, которая должна определить скорость передачи сообщения, правильно декодировать сообщение в точки <code>.</code>, тире <code>-</code> и пробелы (один между символами, три между словами) и возвращать их в виде строки. Обратите внимание, что в начале и конце сообщения могут естественным образом появляться дополнительные <code>0</code>, обязательно игнорируйте их. Также, если вам трудно определить, является ли конкретная последовательность <code>1</code> точкой или тире, предположите, что это точка.</li>
</ol>
<p>2.&nbsp;Функция <code>decodeMorse(morseCode)</code>, которая возьмет результат предыдущей функции и вернет удобочитаемую строку.</p>
<p><strong>ПРИМЕЧАНИЕ:</strong> Для целей кодирования необходимо использовать символы ASCII <code>.</code> и <code>-</code>, а не символы Unicode.</p>
<p>Таблица азбуки Морзе предварительно загружена для вас (см. настройку решения, чтобы получить ее идентификатор на вашем языке).</p>
<pre><code class="language-scala">
<span class="cm-variable">Например</span>:

<span class="cm-variable">morseCodes</span>(<span class="cm-string">".--"</span>) <span class="cm-comment">//для доступа к переводу азбуки Морзе ".--"</span>
</code></pre>
<p>Все тестовые строки будут действительны до такой степени, что их можно будет надежно декодировать, как описано выше, поэтому вы можете пропустить проверку на ошибки и исключения, просто постарайтесь как можно лучше понять, что это за сообщение!</p>
<p>Удачи!</p>
<p>После того, как вы освоите эту ката, вы можете попробовать <a href="http://www.codewars.com/kata/decode-the-morse-code-for-real" data-turbolinks="false" target="_blank">Расшифровать азбуку Морзе, по-настоящему</a>.</p>
</div>